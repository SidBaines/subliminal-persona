#!/bin/bash
# Big v2 run: 18-theme expanded scenarios, ~30x data, 1 seed. NO set -e (vLLM
# segfaults on teardown after writing results); gate on output files.
...
say "Stage A: episodes (20/type x 18 themes x C6/C6e = 720)"
$V sol/agentic.py --model $M --episodes-per-type 20 > sol/logs/big_agentic.log 2>&1 || true

say "Stage B: fork harvest (20 forks/episode)"
$V sol/fork_entries.py --model $M --forks 20 > sol/logs/big_forks.log 2>&1 || true
...
$V sol/measure.py --model $M --traj-dir sol/trajectories_c6 --probes-only \
   --out-prefix c6_ --max-per-cond 40 > sol/logs/big_measure.log 2>&1 || true
$GIT add -A && $GIT commit -qm "big run: episodes + entries + teacher probes (18 themes, ~30x)
...
$GIT add -A && $GIT commit -qm "big run: LoRAs (pushed) + student/misalignment evals + analysis
